    ...
    const cards = [
                    ...
                    <Card>
                        ...
                        <CardContent>
                            ...
                            <p className="my-4"><strong>KVS:</strong> Using <a href="https://docs.aws.amazon.com/kinesisvideostreams/latest/dg/what-is-kinesis-video.html" target="_blank"  className="underline text-primary/80">Amazon Kinesis Video Streams</a> lets us meet requirement <strong>b</strong>, as it offers <a href="https://docs.aws.amazon.com/kinesisvideostreams/latest/dg/API_reader_GetClip.html" target="_blank"  className="underline text-primary/80">GetClip</a> API that allows users to create a clip from a KVS stream for a given time range. It also meets requirement <strong>c</strong> as users can easily change the stream retention period.</p>
                            <p className="my-4"><strong>ECS Fargate:</strong> In order to persist an IP camera stream in KVS, it requires a process that reads from the camera and pushes to KVS. This can be done using this <a href="https://github.com/awslabs/amazon-kinesis-video-streams-producer-sdk-cpp" target="_blank"  className="underline text-primary/80">kvs-producer-sdk</a> written in C++. Containerising this process and running using <a href="https://docs.aws.amazon.com/AmazonECS/latest/developerguide/AWS_Fargate.html" target="_blank"  className="underline text-primary/80">ECS Fargate</a> lets us meet requirement <strong>e</strong> as Fargate is Serverless and can scale in and out horizontally. A custom docker image has been created, which consists of a step that checks any existing ECS jobs for the same stream, a step that sets the timeout for the duration of the operational hour, a step that starts a supervisord process to start and self-heal the process that reads from the camera and pushes to KVS.</p>
                            <p className="my-4"><strong>EventBridge Scheduler:</strong> Requirements <strong>a</strong> and <strong>d</strong> are met using <a href="https://docs.aws.amazon.com/scheduler/latest/UserGuide/what-is-scheduler.html" target="_blank"  className="underline text-primary/80">EventBridge Scheduler</a>, which can create cron schedules that invoke various AWS services. The schedule will start an ECS task with the task definition described above. Creation and modification of schedules are done using an SQS queue and a Lambda function. When a user modifies the schedule from the frontend UI, the UI will immediately display the updated schedule, and in the background, the Lambda function will update the schedule accordingly.</p>
                            <p className="my-4"><strong>NAT Gateway:</strong> Using a <a href="https://docs.aws.amazon.com/vpc/latest/userguide/vpc-nat-gateway.html" target="_blank"  className="underline text-primary/80">NAT Gateway</a> allows us to meet requirement <strong>f</strong>, by unifying the outbound IP to that of the NAT Gateway. Customers can add a certain set of IPs, those of the NAT Gateways, to the Access Control List to use the service while not increasing the security risk.</p>
                        </CardContent>
                    </Card>,
                    <Card>
                        <CardHeader>
                        ...
                        <CardContent>
                            ...
                            <Card className="my-4 p-4">
                                <ol className="list-decimal list-inside space-y-2">
                                    ...
                                    <li>The SQS queue is configured to trigger another Lambda function that creates a clip from the KVS stream using the <a href="https://docs.aws.amazon.com/kinesisvideostreams/latest/dg/API_reader_GetClip.html" target="_blank"  className="underline text-primary/80">GetClip</a> API.</li>
